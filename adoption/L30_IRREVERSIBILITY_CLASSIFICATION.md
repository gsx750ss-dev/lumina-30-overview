...
# LUMINA-30 Irreversibility Classification for Boundary Review ｜ LUMINA-30 境界レビューのための不可逆性分類
...
- Use [Evidence Requirements](L30_EVIDENCE_REQUIREMENTS.md) to show what evidence supports the classification.
- Use the [Practical Boundary Review Pack](L30_PRACTICAL_BOUNDARY_REVIEW_PACK.md) for a compact working set.
- Use the [Pre-Incident Boundary Review — One-Page Practical Sheet](L30_PRE_INCIDENT_BOUNDARY_REVIEW_ONEPAGE.md) before release, deployment, capability exposure, connection, publication, or authority handoff.
- Use the [Consultation and Routing Map](L30_CONSULTATION_ROUTING_MAP.md) when unsure which review path should handle the issue.
...
LUMINA-30の境界問いを適用する前に、どの不可逆な経路が開かれうるかを分類します。
...
- 分類を支える証拠を確認する場合は、[証拠要件](L30_EVIDENCE_REQUIREMENTS.md)を使用します。
- 小さな作業セットとして使う場合は、[実務境界レビューパック](L30_PRACTICAL_BOUNDARY_REVIEW_PACK.md)を使用します。
- 公開、導入、能力露出、接続、不可逆的な公開、権限移譲の前には、[事故前境界レビュー — 1ページ実務シート](L30_PRE_INCIDENT_BOUNDARY_REVIEW_ONEPAGE.md)を使用します。
- どのレビュー経路で扱うべきか迷う場合は、[相談・問いの振り分け地図](L30_CONSULTATION_ROUTING_MAP.md)を使用します。
