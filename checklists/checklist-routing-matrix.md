...
本表は、最適化圧に関する事例を、適切なLUMINA-30レビュー焦点へ振り分けるためのものである。

本表は、分野別の法律・規制・監査・技術レビューを置き換えない。
...
最適化圧によって、不可逆的結果が生じる前に、人間の拒否・修正・移行・回復・再参入の実効条件が失われる可能性がある場合、本表を用いる。

該当する不可逆化種別が存在しない場合、その事例はLUMINA-30境界メカニズムの対象外である可能性がある。
...
| 不可逆化種別 | 典型的な最適化圧 | 失われうるもの | 必要なレビュー焦点 | チェックリスト方向 |
|---|---|---|---|---|
| 技術的不可逆化 | 高速デプロイ、権限拡張、外部ツール使用、CI/CD高速化 | 停止能力、隔離能力、ロールバック能力、権限回収 | 外部影響または不可逆的影響前の人間介入 | 境界／技術制御レビュー |
| 社会的不可逆化 | コスト削減、自動化、プラットフォーム化、制度委任 | 声、生活基盤、実効的異議申立、社会的地位 | 声を失う前の拒否・移行・回復・証拠 | 社会境界レビュー |
| 職業的不可逆化 | 労働置換、タスク自動化、採用経路削除、訓練経路削除 | 技能、職業経路、再参入、職業的同一性 | 置換固定化前の再参入・移行能力 | 職業境界レビュー |
| 制度的不可逆化 | 行政自動化、手続き委任、例外運用の常態化 | 不服申立、人間審査、修正、説明責任 | 制度固定化前の人間審査・修正経路 | 制度レビュー |
| 経済的不可逆化 | 市場集中、金融自動化、ネットワーク効果、勝者総取り型拡大 | 代替手段、市場退出選択肢、損失からの回復 | 対称摩擦、過度集中防止、責任割当 | 経済境界レビュー |
| 情報的不可逆化 | エンゲージメント最適化、自動スコアリング、大量配信、信用評価システム | 訂正、公共的判断、信用回復、認識の健全性 | 透明性、訂正可能性、監査可能性、有害分類の撤回 | 情報健全性レビュー |
| 権限的不可逆化 | 自律判断権限、システムへの委任、指揮命令の高速化 | 人間承認、政治的制御、指揮責任 | 権限保持、多者承認、停止権限 | 権限境界レビュー |
| 回復不能化 | 依存、技能喪失、手動経路の置換、代替手段の削除 | 理論上は可能でも実効的な回復が不可能になること | バックアップ、手動代替、移行経路、回復証拠 | 回復経路レビュー |

---

## Routing Questions ｜ 振り分け質問
...
2. どの人間能力が圧縮されているか。
3. 何が不可逆化しうるか。
4. 誰が拒否・修正・移行・回復・再参入を失いうるか。
5. その喪失はいつ実質的に不可逆化するか。
6. 誰が摩擦設計責任を持つか。
7. 誰が摩擦を運用するか。
8. 声または地位が失われる前に、誰が証拠を保存するか。
9. 誰が実効性を検証するか。
10. その仕組みは迂回可能か。

---

## Proportionality Levels ｜ 比例性レベル
...
| レベル | 条件 | レビュー負担 |
|---|---|---|
| 軽度 | 可逆的、限定的、任意、回復可能 | 最小限の記録と自己確認 |
| 中度 | 明確な集団または制度手続きに影響する | 責任割当と証拠記録 |
| 高度 | 拒否・移行・回復・再参入を消滅させうる | 第三者レビューと迂回防止確認 |
| 重大 | 広域・インフラ・権限・文明境界リスク | 独立レビュー、公的説明責任、固定化前の是正 |
